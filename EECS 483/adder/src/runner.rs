...
mod span {
    ...
    use crate::interp::InterpErr;
    impl<Span> InterpErr<Span> {
        pub fn map_span<F, SpanPrime>(self, f: F) -> InterpErr<SpanPrime>
        where
            F: FnOnce(&Span) -> SpanPrime,
        {
            match self {
		InterpErr::UnboundVar { unbound, location } =>
		    InterpErr::UnboundVar { unbound, location: f(&location) },
		InterpErr::UnsupportedFeature { msg, location } =>
		    InterpErr::UnsupportedFeature { msg, location: f(&location) },
...
pub enum RunnerErr<Span> {
    ...
    Link(String),
    Interp(InterpErr<Span>),
    Run(String),
}

impl<Span> Display for InterpErr<Span>
where
    Span: Display,
{
    ...
    W: std::io::Write,
{
    ...
    interp::prog(&prog, w).map_err(|e| RunnerErr::Interp(e.map_span(|s| span1_to_span2(&info, *s))))
}
